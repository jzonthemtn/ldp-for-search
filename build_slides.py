"""
Build the conference deck.

    python build_slides.py

Writes ldp_for_search_slides.pptx. Re-run it after editing the SLIDES list below.
Speaker notes are attached to every slide in the deck itself.
...
SLIDES = [
    ...
    ("image", {
        "title": "Search quality vs epsilon values",
        "path": "07_epsilon_tradeoff_toy_index.png",
        "caption": "Search quality only returns as the noise comes down",
    ...
    ("bullets", {
       ...
       "privacy tax, it is a trade of individual resolution for population accuracy. No chart "
       "backs this up any more, so carry section 11's result yourself. 480 real WANDS queries "
       "in five segments, every user privatizing independently on their own device, no "
       "readable query anywhere and no trusted intermediate step. Individual recovery per "
       "segment runs 0.020, 0.020, 0.063, 0.070 and 0.030 against chance of 0.014 to 0.031, "
       "so one to two and a half times chance, which is not usable. Segment identification is "
       "5 of 5, exact. Same data, same epsilon of 1.0, two different questions. If anyone "
       "wants the convergence behind it, the error falls as one over root n across four "
       "orders of magnitude and the notebook plots it."),
    ...
    ("bullets", {
       ...
       "error goes as one over epsilon times one over root n, so the threshold scales as one "
       "over epsilon squared, which puts it at about 9,200 users at epsilon 0.5 and about 575 "
       "at epsilon 2. "
       ...
       "you actually run. Step three is the third bullet here. Step four "
    ...
    ("statement", {
        "text": "LDP does not cost you search tuning.\nIt costs you exact queries.",
    }, "The reframe. Every input relevance tuning actually needs is a population statistic. "
       "Say queries rather than people, because the identifiers are still in the clear and "
       "the limitations slide admits it shortly. Exact is doing work too, since the category "
       "still leaks at a usable epsilon, which is the first limitation you show. Let "
    ...
    ("bullets", {
        ...
        "bullets": [
            ...
            "What they cannot do is show you a specific user's session",
        ],
    ...
    ("table", {
        ...
        "lead": "The labels were never protected, so what changes is what you key them on",
        "columns": ["", "Classic LTR", "Under LDP"],
        "rows": [
    ...
    }, "This expands the learning-to-rank bullet on the previous slide, and it is the one place "
       "to be careful, because it is a recipe rather than a result. Nothing in this talk trains "
       "a ranker. Say so if asked. What is measured is the inputs: clicks were never touched by "
       "the mechanism, and the segment centroid is accurate above roughly 2,300 users. The "
       "labels are therefore exactly what they are today. What moves is the key. You cannot "
       "read a query, so you cannot build judgment lists per query string, and you cannot "
       "recompute BM25 against words you do not have. You group by segment instead and use its "
       "centroid as the query representation, which suits an embedding ranker and suits a "
       "term-based feature set badly. Walk the example slowly, it is the whole slide: a few "
       "thousand people search for beds, every one of them privatizes on their own device, "
       "and the server averages what arrives into a centroid that is accurate but belongs to "
       "nobody. The clicks sitting next to it were never noised. That pair, the centroid and "
       "the clicks, is a training example. Beds is the cohort behind the convergence plot, so "
    ...
    ("title", {
        ...
        "subtitle_url": "https://github.com/jzonthemtn/ldp-for-search",
        "footer": "Jeff Zemerick",
        "event": "OpenSearchCon NA",
        "date": "September 24, 2026",
        "qr": FEEDBACK_QR,
...
def render_title(prs, d):
    ...
    if d.get("subtitle_url"):
        r2.hyperlink.address = d["subtitle_url"]
